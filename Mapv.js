...
util.extend(DataRangeControl.prototype, {
    ...
    drawCategorySplit: function drawCategorySplit(splitList, drawOptions) {
        var canvas = this.canvas;
        var ctx = canvas.getContext('2d');
		var tot_keys = Object.keys(splitList).length;
        canvas.width = Math.ceil(tot_keys / 8) * 70 + 10;
		console.log(canvas.width);
		if (tot_keys > 8) {
			canvas.height = 8 * 15 + 5;
		} else {
			canvas.height = tot_keys * 15 + 5;
		}
        ...
        for (var key in splitList) {
            ...
            ctx.beginPath();
            ctx.arc(column * 70 + 10, row * 15 + 10, 5, 0, Math.PI * 2, false);
            ctx.closePath();
            ctx.fill();
            ctx.fillStyle = '#333';
            ctx.fillText(key, column * 70 + 20, row * 15 + 15);
            i++;
			row++;
			if (row == 8) {
				row = 0;
